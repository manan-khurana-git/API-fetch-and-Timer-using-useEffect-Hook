...
const Datafetch = () => {
  ...
  const fetchdata = async () => {
    try {
      ...
      console.log("Datafetched", res.data.products);
          ...
          {data.map((item, index) => {
            return (
              <div className="border m-2 p-2 ">
                ...
                <p className="border rounded-md m-2 p-2 text-black bg-teal-300">{item.description}</p>
                <div className='flex justify-between p-2 m-2'>
                  <p className='bg-pink-300 p-2 rounded-md text-black'>Price: {item.price}</p>
